...
def createInputWindow(gdb):
	...
	inputs = sg.Column([[sg.Input('', size=(30,1))] for h in gdb.headers ])
	layout = [[names, inputs]]
...
def GUI(gdb):
	main_window = sg.Window('Game Database', layout=createLayout(gdb), font=('Helvetica', 14), finalize=True)
	add_window = None	#add new record window
	edit_window = None	#edit exsiting record window
	...
	while True:
		...
		if event == sg.WIN_CLOSED:
			window.close()
			if window == edit_window:       # if closing win 2, mark as closed
				edit_window = None
			elif window == add_window:       # if closing win 2, mark as closed
				add_window = None
			elif window == main_window:     # if closing win 1, exit program
				break
		
		elif window == add_window:	#ADD window
			if event == "Add":
				gdb.add_record(list(values.values())) #values is a dict with keys 0,1,2,3,..,len(inputs)
				updateTable(main_window, sortby, sortdir)
			window.close()  #close add_window anyway
			add_window = None
		
		elif window == edit_window:	#EDIT window
			if event == "Apply":
				gdb.remove_record(edited_id)			#delete the old one
				gdb.add_record(list(values.values()))	#add the new one
				updateTable(main_window, sortby, sortdir)
			window.close() 								#close edit_window anyway
			edit_window = None
		
		elif window == main_window:
			if event == 'Add record' and not add_window:	#dont create window if its already open
				add_window = createInputWindow(gdb)
			...
			elif event == "Edit record" and not edit_window:	#dont create window if its already open
				...
				if len(selected_ids) > 0: 					#at least one selected
					...
					edited_id = real_id
					edit_window = createEditWindow(gdb, real_id)
				else:
					print("-select a table record first")
	window.close()

def updateTable(main_window, sortby, sortdir):
	main_window['-TABLE-'].update(values = createTableData(gdb, sortby, sortdir))
